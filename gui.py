...
class ReminderGUI(QWidget):
    ...
    def on_add_interval_clicked(self):
        ...
        if not title or not interval or not message:
            QMessageBox.warning(self, "错误", "请填写有效的标题、提醒内容和时间间隔")
            return
        minutes = int(interval)
        job_id = add_interval_reminder(minutes, title, message)
        self.interval_list.addItem(f"[{job_id}] [{title}] 每 {minutes} 分钟：{message}")
        QMessageBox.information(self, "成功", f"已添加 [{title}] 每 {minutes} 分钟提醒：{message}")
    ...
    def on_add_daily_clicked(self):
        ...
        if not title or not hour_str or not minute_str or not message:
            QMessageBox.warning(self, "错误", "请填写有效的标题、时间和提醒内容")
            return
        hour = int(hour_str)
        minute = int(minute_str)
        job_id = add_daily_reminder(hour, minute, title, message)
        self.daily_list.addItem(f"[{job_id}] [{title}] 每天 {hour:02d}:{minute:02d}：{message}")
        QMessageBox.information(self, "成功", f"已添加 [{title}] 每天 {hour:02d}:{minute:02d} 提醒：{message}")